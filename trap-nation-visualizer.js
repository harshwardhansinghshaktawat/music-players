...
//  Full beat detection, 3D particles, ring visualizer
//  Integrated player strip with auto-hide (YouTube style)
// ============================================================
...
class TrapNationVisualizer extends HTMLElement {
    constructor() {
        ...
        this._S = {
            ...
            glow: '#00ffff', bgTint: '#001122'
        };

        // Particle config
        this._particleCount = 420;
        this._particleMaxSize = 1.1;
        this._particleSpeed = 1.0;

        // Dimensions
        this._W = 800; this._H = 600;
        this._CX = 400; this._CY = 300;
        this._BASE_R = 0; this._MAX_H = 0; this._CENTER_R = 0;

        // Animation
        this._animId = null;
        this._particles = [];
        this._bgImg = null; this._bgReady = false;
        this._coverImg = null; this._coverReady = false;

        // Canvases
        this._bgC = null; this._mc = null;
        this._off1 = null; this._off2 = null;
        this._bg = null; this._ctx = null;
        this._o1 = null; this._o2 = null;

        // Seeking
        this._seeking = false;
        this._audioEl = null; // <audio> element for playback
    }

    static get observedAttributes() {
        return [
            'player-data',
            'scheme-c1', 'scheme-c2', 'scheme-c3', 'scheme-glow', 'scheme-bgtint',
    ...
    attributeChangedCallback(name, oldVal, newVal) {
        ...
        if (name === 'player-data' && newVal) {
            ...
            } catch (e) { console.error('[TrapNation] Data error:', e); }
        } else if (name.startsWith('scheme-')) {
            const key = name.replace('scheme-', '').replace(/-([a-z])/g, (_, c) => c.toUpperCase());
            const map = { c1: 'c1', c2: 'c2', c3: 'c3', glow: 'glow', bgtint: 'bgTint' };
            const prop = name.replace('scheme-', '');
            if (map[prop]) this._S[map[prop]] = newVal;
...
.tnv-progress-fill::after {
    ...
    box-shadow: 0 0 6px var(--tnv-c1, #00ffff);
}
...
.tnv-song-title {
    ...
    text-shadow: 0 0 10px var(--tnv-c1, #00ffff);
}
.tnv-song-artist {
    font-size: 10px;
...
.tnv-btn:hover { color: #fff; background: rgba(255,255,255,0.1); }
.tnv-btn.active { color: var(--tnv-c1, #00ffff); }
.tnv-btn svg { width: 18px; height: 18px; fill: currentColor; }
.tnv-play-btn {
    width: 42px; height: 42px;
    background: var(--tnv-c1, #00ffff);
    color: #000 !important;
    box-shadow: 0 0 16px var(--tnv-c1, #00ffff);
}
.tnv-play-btn:hover {
    background: #fff !important;
    transform: scale(1.08);
}
.tnv-play-btn svg { width: 20px; height: 20px; fill: #000; }
...
.tnv-drawer-search svg { width: 14px; height: 14px; fill: rgba(255,255,255,0.3); flex-shrink: 0; }
.tnv-search-input {
    flex: 1; background: transparent; border: none; outline: none;
    color: #fff; font-size: 11px; letter-spacing: 1px;
    font-family: 'Courier New', monospace;
}
.tnv-search-input::placeholder { color: rgba(255,255,255,0.25); }
.tnv-drawer-body { flex: 1; overflow-y: auto; padding: 6px; }
.tnv-drawer-body::-webkit-scrollbar { width: 4px; }
...
.tnv-queue-song.active { background: rgba(255,255,255,0.08); }
.tnv-queue-num {
    font-size: 10px; color: rgba(255,255,255,0.3);
    width: 20px; text-align: center; flex-shrink: 0; letter-spacing: 1px;
}
.tnv-queue-song.active .tnv-queue-num { color: var(--tnv-c1, #00ffff); }
.tnv-queue-art {
...
.tnv-queue-song.active .tnv-queue-title { color: var(--tnv-c1, #00ffff); }
.tnv-queue-artist {
    font-size: 10px; color: rgba(255,255,255,0.4);
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    letter-spacing: 1px;
}
.tnv-queue-dur { font-size: 10px; color: rgba(255,255,255,0.3); flex-shrink: 0; }
.tnv-queue-empty { padding: 30px 16px; text-align: center; color: rgba(255,255,255,0.25); font-size: 11px; letter-spacing: 2px; }

/* Responsive */
@media (max-width: 480px) {
    .tnv-strip { padding: 0 12px 10px; }
...
<div class="tnv-wrap" id="tnvWrap">
    ...
    <div class="tnv-strip" id="tnvStrip">
        ...
        <div class="tnv-controls-row">
            ...
            <div class="tnv-controls">
                ...
                <button class="tnv-btn" id="tnvRepeatBtn" title="Repeat Off">
            ...
            <button class="tnv-btn tnv-queue-btn" id="tnvQueueBtn" title="Queue">
    ...
    <div class="tnv-drawer" id="tnvDrawer">
        <div class="tnv-drawer-header">
            ...
            <button class="tnv-btn" id="tnvDrawerClose" style="width:28px;height:28px">
    ...
    _buildParticles() {
        this._particles = Array.from({ length: this._particleCount }, () => this._newParticle(true));
    }
    ...
    _killSrc() {
        if (this._srcNode) {
            try { this._srcNode.disconnect(); } catch (e) { }
            this._srcNode = null;
        }
        this._live = false;
    }
    ...
    _getPeaksAtThreshold(data, threshold, sampleRate) {
        const peaks = [];
        const skip = Math.floor(sampleRate * 0.23);
        let i = 0;
        while (i < data.length) {
    ...
    _clearBeatState() {
        this._kickTimestamps = []; this._nextKickIdx = 0;
        this._detectedBPM = 0; this._micMode = false;
        this._kickStrength = 0; this._bgPulse = 0;
        this._kickEnv = 0; this._bgEnv = 0;
    }

    _resetRTBeat() {
        this._rtHistory.fill(0); this._rtIdx = 0;
        this._rtFilled = false; this._rtWarmup = 0; this._rtCooldown = 0;
    ...
    _tickRTBeat() {
        ...
        let avg = 0; for (let i = 0; i < len; i++) avg += this._rtHistory[i]; avg /= len;
    ...
    _drawBG() {
        const bg = this._bg, W = this._W, H = this._H, CX = this._CX, CY = this._CY;
        ...
        if (this._bgReady) {
            ...
            bg.drawImage(this._bgImg, CX - this._bgImg.naturalWidth * sc / 2 + this._kbX,
                CY - this._bgImg.naturalHeight * sc / 2 + this._kbY,
                this._bgImg.naturalWidth * sc, this._bgImg.naturalHeight * sc);
            bg.restore();
        ...
        if (this._bgPulse > 0.04) {
            ...
            gr.addColorStop(0, this._hexToRgba(this._S.c1, a * 0.55));
            gr.addColorStop(0.35, this._hexToRgba(this._S.bgTint, a * 0.35));
            gr.addColorStop(0.70, this._hexToRgba(this._S.bgTint, a * 0.12));
            gr.addColorStop(1, 'rgba(0,0,0,0)');
    ...
    _drawRing() {
        ...
        const BASE_R = this._BASE_R, MAX_H = this._MAX_H;
        const S = this._S, rs = this._ringSmooth;
        ...
        for (let i = 0; i < this._N_RING; i += 2) {
            ...
            const ang = (i / this._N_RING) * Math.PI * 2 - Math.PI / 2;
            const r = BASE_R + v * MAX_H;
        ...
        ctx.restore();
        ctx.save(); ctx.strokeStyle = S.c1 + '22'; ctx.lineWidth = 1;
        ctx.beginPath(); ctx.arc(CX, CY, BASE_R, 0, Math.PI * 2); ctx.stroke(); ctx.restore();
    }
    ...
    _drawCenter() {
        ...
        ctx.lineWidth = 8; ctx.shadowBlur = 20 + this._kickStrength * 28; ctx.shadowColor = S.glow;
        ctx.globalAlpha = 0.25 + this._kickStrength * 0.35;
        ctx.beginPath(); ctx.arc(CX, CY, cr + 4, 0, Math.PI * 2); ctx.stroke(); ctx.restore();

        ctx.save(); ctx.strokeStyle = S.c1; ctx.lineWidth = 3.5;
        ctx.shadowBlur = 22 + this._kickStrength * 30; ctx.shadowColor = S.glow;
        ctx.globalAlpha = 0.88 + this._kickStrength * 0.12;
    ...
    async _loadSong(index, autoPlay = false) {
        ...
        this._updateCoverFromSong(song);
        ...
        // Use <audio> element for streaming playback + offline analysis
        this._killSrc();
        this._audioEl.pause();
        ...
        // Fetch and analyze for beat detection
        try {
            const res = await fetch(song.audioFile);
            const buf = await res.arrayBuffer();
            const decoded = await this._aC.decodeAudioData(buf.slice(0));
            this._songDuration = decoded.duration;
            await this._analyzeFile(decoded);
        } catch (e) {
            console.warn('[TrapNation] Beat analysis failed, using RT mode', e);
            this._micMode = true;
        }
        ...
        if (autoPlay) {
            this._audioEl.play().catch(e => console.warn('Autoplay blocked:', e));
        }
    }

    _updateSongInfoUI(song) {
        const title = this.querySelector('#tnvSongTitle');
        const artist = this.querySelector('#tnvSongArtist');
        if (title) title.textContent = (song.title || 'Unknown Title').toUpperCase();
    ...
    _togglePlay() {
        ...
        if (this._audioEl.paused) {
            ...
            this._audioEl.play().catch(e => console.warn('Play error:', e));
        } else {
            this._audioEl.pause();
        }
    }
    ...
    _toggleMute() {
        if (this._volume > 0) {
            this._lastVolume = this._volume; this._volume = 0;
        } else {
            this._volume = this._lastVolume || 0.8;
        }
        if (this._audioEl) this._audioEl.volume = this._volume;
        const slider = this.querySelector('#tnvVolSlider');
    ...
    _onTimeUpdate() {
        ...
        const fill = this.querySelector('#tnvProgressFill');
        const timeEl = this.querySelector('#tnvCurTime');
        if (fill) fill.style.width = pct + '%';
        if (timeEl) timeEl.textContent = this._fmtTime(cur);
    }

    _onMetadata() {
        const dur = this._audioEl.duration;
        if (dur && !isNaN(dur)) {
            const el = this.querySelector('#tnvTotalTime');
            if (el) el.textContent = this._fmtTime(dur);
        }
    }

    _onEnded() {
        if (this._repeat === 'one') {
            this._audioEl.currentTime = 0; this._audioEl.play();
        } else {
            this._next();
        }
    }
    ...
    _toggleDrawer() {
        ...
        const drawer = this.querySelector('#tnvDrawer');
        const btn = this.querySelector('#tnvQueueBtn');
    ...
    _renderDrawer() {
        ...
        if (filtered.length === 0) {
            body.innerHTML = '<div class="tnv-queue-empty">NO SONGS FOUND</div>'; return;
        }
        body.innerHTML = filtered.map((song, i) => {
            const actualIdx = this._songs.indexOf(song);
            const active = actualIdx === this._currentSong;
            const art = song.coverImage ? `<img src="${this._esc(song.coverImage)}" alt="">` : '';
            return `<div class="tnv-queue-song${active ? ' active' : ''}" data-index="${actualIdx}">
        ...
        body.querySelectorAll('.tnv-queue-song').forEach(row => {
            row.addEventListener('click', () => {
                this._loadSong(parseInt(row.dataset.index), true);
                this._toggleDrawer();
            });
        });
    }
    ...
    _bindUIEvents() {
        // Mouse auto-hide (YouTube style)
        const wrap = this.querySelector('#tnvWrap');
        const strip = this.querySelector('#tnvStrip');

        const showUI = () => {
            this._uiVisible = true;
            wrap.classList.add('ui-visible');
            strip.classList.remove('hidden');
            clearTimeout(this._hideTimer);
            this._hideTimer = setTimeout(() => {
                strip.classList.add('hidden');
                wrap.classList.remove('ui-visible');
        ...
        this.querySelector('#tnvSearch').addEventListener('input', e => {
            this._searchQuery = e.target.value.toLowerCase();
            this._renderDrawer();
        });

        // Progress bar
        const pb = this.querySelector('#tnvProgressBar');
        ...
        document.addEventListener('click', e => {
            const drawer = this.querySelector('#tnvDrawer');
            const qBtn = this.querySelector('#tnvQueueBtn');
            if (drawer && this._drawerOpen && !drawer.contains(e.target) && !qBtn.contains(e.target)) {
                this._toggleDrawer();
            }
        });

        // Apply CSS variable for accent color
        this._applyAccentVar();
    }

    _applyAccentVar() {
        this._wrap = this.querySelector('#tnvWrap');
        if (this._wrap) {
            this._wrap.style.setProperty('--tnv-c1', this._S.c1);
        }
    }

    // Override scheme setter to also update CSS var
    set scheme(s) {
        this._S = s;
        this._applyAccentVar();
    }
