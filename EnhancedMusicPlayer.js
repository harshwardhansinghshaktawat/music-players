class EnhancedMusicPlayer extends HTMLElement {
                ...
                <div class="player-layout">
                    ...
                    <div class="info-section">
                        ...
                        <div class="social-share-container">
                            <div class="social-share-title">Share this song</div>
    ...
    _initializeWaveSurfer() {
        ...
        this._audioElement.addEventListener('loadedmetadata', () => {
            ...
            this._shadow.querySelector('.total-time').textContent = this._formatTime(this._audioElement.duration);
        });
        ...
        if (this._playerData && this._playerData.songs.length > 0) {
    ...
    render() {
        ...
        // Update UI elements
        this._shadow.querySelector('.title').textContent = song.title || 'Unknown Title';
        this._shadow.querySelector('.artist').textContent = song.artist || 'Unknown Artist';
    ...
    _toggleMute() {
        if (this._audioElement) {
            if (this._audioElement.volume > 0) {
                ...
                this._setVolume(0);
                this._shadow.querySelector('.volume-slider').value = 0;
            } else {
                const volumeToSet = this._lastVolume || 0.8;
                this._setVolume(volumeToSet);
                this._shadow.querySelector('.volume-slider').value = volumeToSet;
            }
        }
    }
    ...
    _updateCurrentTime() {
        ...
        this._shadow.querySelector('.current-time').textContent = this._formatTime(currentTime);
        
        // Update progress bar
        const progress = this._audioElement.currentTime / this._audioElement.duration;
        if (!isNaN(progress)) {
            this._shadow.querySelector('.progress-current').style.width = `${progress * 100}%`;
            this._shadow.querySelector('.progress-indicator').style.left = `${progress * 100}%`;
        }
    }
    ...
    _updateDuration() {
        ...
        if (!isNaN(duration)) {
            this._shadow.querySelector('.total-time').textContent = this._formatTime(duration);
        }
    }
    ...
    _updateStreamingLinks(song) {
        ...
        if (song.streamingLinks) {
            for (const [service, link] of Object.entries(links)) {
                if (song.streamingLinks[service]) {
                    link.href = song.streamingLinks[service];
