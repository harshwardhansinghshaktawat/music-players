class EnhancedMusicPlayer extends HTMLElement {
    ...
    _initializeWaveSurfer() {
        ...
        // THIS IS THE FIXED CODE FOR THE AUTO-PLAY ISSUE
        this._audioElement.addEventListener('ended', () => {
            console.log("Song ended, playing next...");
            // Don't set playing state to false here - that was causing the issue
            this._stopVisualization();
            
            // Auto play next if not in repeat mode
            if (!this._isRepeat) {
                // Force next song to play by passing true as the second parameter
                this._changeSong(1, true);
            } else {
                // For repeat mode, play the same song again
                this._audioElement.currentTime = 0;
                this._audioElement.play().catch(e => console.error("Error replaying song:", e));
            }
        });
            ...
            playBtn.addEventListener('click', () => {
                if (this._audioElement) {
                    if (this._audioElement.paused) {
                        this._audioElement.play().catch(e => console.error("Error playing audio:", e));
                    } else {
            ...
            this._playerData.songs.forEach((song, index) => {
                ...
                songItem.addEventListener('click', () => {
                    ...
                    if (wasPlaying && this._audioElement) {
                        this._audioElement.play().catch(e => console.error("Error playing new song:", e));
                    }
                });
    ...
    // FIXED: Added forcePlay parameter to ensure next song plays automatically
    _changeSong(direction, forcePlay = false) {
        ...
        const wasPlaying = forcePlay || this._isPlaying;
        ...
        console.log(`Changing song from ${this._playerData.currentIndex} to ${newIndex}`);
        this._playerData.currentIndex = newIndex;
        this.render();
        
        // Always auto-play the new song if the previous one was playing or forcePlay is true
        if (wasPlaying && this._audioElement) {
            this._audioElement.play().catch(e => console.error("Error playing next song:", e));
        }
    }
