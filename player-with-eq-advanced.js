// ============================================================
//  ADVANCED MUSIC PLAYER PRO — Wix Blocks Custom Element
//  FIXED VERSION:
//  ✅ FIX 1: Player name now updates from panel
//  ✅ FIX 2: WCAG AA compliant default colors
//  ✅ FIX 3: EQ slider knobs perfectly centered
//  ✅ FIX 4: All color themes WCAG compliant
// ============================================================

class AdvancedMusicPlayerPro extends HTMLElement {
    constructor() {
        super();
        this._audio           = null;
        this._ctx             = null;
...
advanced-music-player-pro {
    display:block;
    width:100%;
    height:100%;
    box-sizing:border-box;
    font-family:var(--body-font, 'Barlow Condensed', sans-serif);
...
advanced-music-player-pro *, 
advanced-music-player-pro *::before, 
advanced-music-player-pro *::after {
...
.amp-brand {
    ...
    color:var(--acc); font-family:var(--title-font);
}
.amp-dot {
...
.amp-ibtn.on     { color:var(--acc); border-color:var(--acc); background:rgba(59,130,246,.12); }
...
.amp-play:hover  { background:var(--acc2); box-shadow:0 0 14px rgba(59,130,246,.4); transform:scale(1.04); }
...
/* FIX 3: Vertical slider with perfectly centered thumb */
input.amp-eq-sl {
...
/* Chrome/Safari/Edge - Vertical track */
input.amp-eq-sl::-webkit-slider-runnable-track {
    width: 3px;
    height: 100%;
    background: var(--b2);
    border-radius: 2px;
...
/* Chrome/Safari/Edge - Centered thumb */
input.amp-eq-sl::-webkit-slider-thumb {
    ...
    border-radius: 2px;
    cursor: pointer;
    margin-left: -4.5px; /* Centers the 12px thumb on the 3px track */
}

/* Firefox - Vertical track */
input.amp-eq-sl::-moz-range-track {
    width: 3px;
    background: var(--b2);
    border-radius: 2px;
    cursor: pointer;
...
/* Firefox - Centered thumb */
input.amp-eq-sl::-moz-range-thumb {
    width: 12px;
    height: 6px;
    background: var(--acc);
    border: none;
...
.amp-lib-title {
    ...
    text-transform:uppercase; color:var(--acc); font-family:var(--title-font);
}
.amp-lib-tabs {
    display:flex;
    background:var(--panel); border-bottom:1px solid var(--b1);
...
.amp-lib-srch {
    ...
    font-size:11px; font-family:var(--body-font);
...
<!-- TOP BAR -->
<div class="amp-top">
    <span class="amp-brand">AMP Pro</span>
...
<div class="amp-lib" id="amp-lib">
    ...
    _accentColor() {
        return getComputedStyle(this).getPropertyValue('--acc').trim() || '#3b82f6';
    }

    // FIX 2 & 4: Color theming with proper WCAG mapping
    _applyColors() {
        const map = {
            'primary-color':    '--acc',
...
customElements.define('advanced-music-player-pro', AdvancedMusicPlayerPro);
