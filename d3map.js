class D3WorldMapElement extends HTMLElement {
  ...
  handleResize() {
    ...
    this.resizeTimeout = setTimeout(() => {
      ...
      // Recalculate projection with larger scale - map touches edges
      ...
      // Reset zoom to identity transform on resize
      if (this.zoom) {
  ...
  async initializeMap() {
    ...
    // Calculate scale to make map bigger - touches top/bottom edges
    const scale = Math.min(width / 5, height / 2.8);
